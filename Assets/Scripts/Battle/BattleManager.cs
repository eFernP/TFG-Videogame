...
public class BattleManager : MonoBehaviour
{
    ...
    public InformationPanel InformationMessage;
    ...
    void Start()
    {
        ...
        GameObject hero = GameObject.FindGameObjectsWithTag("Player")[0];
        AudioManager = hero.GetComponent<Audio>();
        playerBreathManager = hero.GetComponent<BreathManager>();
        BossText = GameObject.Find("Subtitle").GetComponent<Subtitles>();
    ...
    void Update()
    {
        if (BlackScreenScript.getOpacity() > 0 && !isSceneReady)
        {
    ...
    {
        if(collider.tag == "Player" && !hasBattleStarted)
        {
            currentPhase = 1;
